...
describe('PictureDb', () => {
    ...
    const pictureTbl = new PictureTbl(<PictureTbl>{
    ...
    it('update & select', async () => {
        pictureTbl.name += 'u'
        pictureTbl.ext += 'u'
        pictureTbl.size += 1024
        pictureTbl.height += 10
        pictureTbl.width += 10
        pictureTbl.content = Buffer.from('1', 'hex')
        const res = await pictureDb.update(pictureTbl)
        expect(res).to.be.true
        const select = await pictureDb.select(pictureTbl.picture_id)
