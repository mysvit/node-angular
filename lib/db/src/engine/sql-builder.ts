...
export namespace SqlBuild {
    ...
    export function limitBuild(limit: SelectLimit) {
        let result: LimitResult = <LimitResult>{
            limit: '',
            values: []
        }
        if (limit) {
            result.limit = ' LIMIT ?'
            result.values.push(limit.offset)
            result.limit += ', ?'
            result.values.push(limit.fetch)
        }
        return result
    }
