...
runLocalBuild() {
  ...
  tmux select-pane -t build:0.5 && tmux send-keys -t build:0.5 "npm start"     Enter && sleep 0.1 #server debug
  tmux select-pane -t build:0.6 && tmux send-keys -t build:0.6 "npm start"     Enter && sleep 0.1 #angular
}
...
runDockerBuild() {
  ...
  tmux send-keys -t build:0.5 "docker run -it --rm --name debug-server -v $(pwd):$(pwd) -w $(pwd)            --net dev-net -p 3000:3000 -p 9229:9229 devnode npm start" Enter && sleep 0.1
