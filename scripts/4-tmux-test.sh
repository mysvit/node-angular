...
runDockerTest() {
  tmux select-pane -t test:0.0 -T 'DTO'     && tmux send-keys -t test:0.0 "docker run -it --rm --name test-dto    -v $(pwd):/$(pwd) -w $(pwd)/lib/dto    --net dev-net -p 9119:9119 devnode npm run test" Enter && sleep 5
  tmux select-pane -t test:0.1 -T 'Shared'  && tmux send-keys -t test:0.1 "docker run -it --rm --name test-shared -v $(pwd):/$(pwd) -w $(pwd)/lib/shared --net dev-net -p 9119:9119 devnode npm run test" Enter && sleep 5
